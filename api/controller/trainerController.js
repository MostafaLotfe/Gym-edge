...
exports.getAllTrainers = async (req, res) => {
  try {
    const features = new APIfeatures(Trainer.find(), req.query)
      .filter()
      .sort()
      .limit()
      .paginate();
...
exports.getOneTrainer = async (req, res) => {
  try {
    const trainer = await Trainer.findOne({ email: req.params.id });
    const private = await privateSubscription.find({ trainer: trainer._id });
